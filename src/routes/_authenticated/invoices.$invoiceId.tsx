...
import { ArrowRight, Banknote, Download, Loader2, Pencil, Printer, ReceiptText } from "lucide-react";
...
export const Route = createFileRoute("/_authenticated/invoices/$invoiceId")({
  head: () => ({ meta: [
    { title: "تفاصيل الفاتورة | الرشودي للعقارات العقارية" },
    { name: "description", content: "عرض الفاتورة وبنودها والمدفوعات المسجلة." },
    { property: "og:title", content: "تفاصيل الفاتورة | الرشودي للعقارات العقارية" },
...
function InvoiceDetailPage() {
  ...
  return <>
    ...
    <div className="flex flex-wrap items-center justify-between gap-3"><Link to="/invoices" className="inline-flex h-10 items-center gap-2 rounded-lg border border-border bg-card px-4 text-[13px] font-semibold"><ArrowRight className="size-4" />رجوع للفواتير</Link><div className="flex gap-2"><button type="button" onClick={() => window.print()} className="inline-flex h-10 items-center gap-2 rounded-lg border border-border bg-card px-3 text-[12.5px] font-semibold"><Printer className="size-4" />طباعة</button><button type="button" onClick={() => void exportInvoice()} className="inline-flex h-10 items-center gap-2 rounded-lg border border-border bg-card px-3 text-[12.5px] font-semibold"><Download className="size-4" />Excel</button><Link to="/invoice-form" search={{ id: invoiceId, ownerId: "" }} className="inline-flex h-10 items-center gap-2 rounded-lg border border-border bg-card px-3 text-[12.5px] font-semibold"><Pencil className="size-4" />تعديل</Link>{remaining > 0 ? <button type="button" onClick={() => { setPayment((current) => ({ ...current, amount: String(remaining) })); setPaymentOpen(true); }} className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-[12.5px] font-semibold text-primary-foreground"><Banknote className="size-4" />تسجيل دفعة</button> : null}</div></div>
