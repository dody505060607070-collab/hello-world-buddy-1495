  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      if (mode === "offer") {
        ...
        const result = await supabase.from("supply_requests").insert({ full_name: form.full_name, phone: form.phone, request_type: form.purpose, city: form.city || null, districts: form.district || null, property_type: form.property_type || null, requester_type: isBroker ? "broker" : "client", broker_name: isBroker ? form.broker_name || null : null, broker_phone: isBroker ? form.broker_phone || null : null, budget_min: form.budget_min ? Number(form.budget_min) : null, budget_max: form.budget_max ? Number(form.budget_max) : null, requester_notes: form.description || null });
        if (result.error) throw result.error;
      }
      try { const { reportPublicRequest } = await import("@/lib/automation.functions"); await reportPublicRequest({ data: { full_name: form.full_name, phone: form.phone, purpose: form.purpose, city: form.city || undefined, property_type: form.property_type || undefined } }); } catch { /* الإرسال محفوظ حتى عند تعذر التنبيه */ }
